...
description: Explore a estrutura da tabela sales_order no Data Warehouse da Commerce Intelligence. Entenda como cada linha representa uma ordem e onde ocorrem divisões personalizadas.
...
| `store_id` | `Foreign key` associado à tabela `store`. Ingressar em `store`.`store_id` para determinar qual visualização de loja da Commerce está associada ao pedido |
...
| **Nome da coluna** | **Descrição** |
|---|---|
| `Billing address city` | Cidade de cobrança do pedido. Calculado ingressando em `sales_order`.`billing_address_id` para `sales_order_address`.`entity_id` e retornando o campo `city` |
| `Billing address country` | Código do país para cobrança do pedido. Calculado ingressando em `sales_order`.`billing_address_id` para `sales_order_address`.`entity_id` e retornando o `country_id` |
| `Billing address region` | Região de cobrança (geralmente estado ou província) do pedido. Calculado ingressando em `sales_order`.`billing_address_id` para `sales_order_address`.`entity_id` e retornando o campo `region` |
| `Customer's first order date` | Carimbo de data e hora do primeiro pedido feito por esse cliente. Geralmente considerada como a &quot;data de aquisição&quot; de um cliente. Calculado retornando o mínimo de `sales_order`.`created_at` para cada cliente único |
...
| `Customer's group code` | Nome do grupo do cliente que fez este pedido. Calculado ingressando em `sales_order`.`customer_group_id` para `customer_group`.`customer_group_id` e retornando o campo `customer_group_code` |
...
| `Number of items in order` | Quantidade total de itens incluídos no pedido. Calculado ingressando em `sales_order`.`entity_id` para `sales_order_item`.`order_id` e soma de `sales_order_item`.`qty_ordered` campo |
...
| `Shipping address city` | Cidade de remessa do pedido. Calculado ingressando em `sales_order`.`shipping_address_id` para `sales_order_address`.`entity_id` e retornando o campo `city` |
| `Shipping address country` | Código do país para remessa do pedido. Calculado ingressando em `sales_order`.`Shipping_address_id` para `sales_order_address`.`entity_id` e retornando o `country_id` |
| `Shipping address region` | Região de remessa (geralmente estado ou província) do pedido. Calculado ingressando em `sales_order`.`shipping_address_id` para `sales_order_address`.`entity_id` e retornando o campo `region` |
| `Store name` | O nome da loja da Commerce associada a este pedido. Calculado ingressando em `sales_order`.`store_id` para `store`.`store_id` e retornando o campo `name` |
